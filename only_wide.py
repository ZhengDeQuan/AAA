...
class TFLearnWideAndDeep(object):
    ...
    def __init__(self, model_type="wide+deep", verbose=None, name=None, tensorboard_verbose=3, 
                 wide_learning_rate=0.001, deep_learning_rate=0.001, checkpoints_dir=None,tensorboard_dir=None,max_checkpoints=15,
    ...
    def read_embedding(self,embedding_path):
        ...
        for idx , wd in enumerate(words):
            vocab[wd] = idx
        vocab["[UNK]"] = len(vocab)
        df.loc[df.shape[0] + 1] = numpy.zeros(df.shape[1],dtype=float32)
        return vocab, df
    ...
    def deep_model(self, wide_inputs,deep_inpus, n_inputs, n_nodes=[100, 50], use_dropout=False):
        ...
        '''
        self.vocab, self.embedding = self.read_embedding(embedding_path="I haven't deal with it")
        self.embedding = np.array(self.embedding, dtype=np.float32)

        net = zqtflearn.layers.embedding_ops.embedding(deep_inpus, len(self.vocab), self.embedding.shape[1], trainable=False
                                                       , name="deep_video_ids_embed" % cc)

        network = tf.concat([wide_inputs,net] , axis = 1, name="deep_concat") #x=xigma(dim of each element in flat_vars) + wide_inputs.size(1) [?,x]
    ...
    def wide_model(self, inputs, n_inputs):
        ...
        network = zqtflearn.fully_connected(network, n_inputs, activation="linear", name="wide_linear", bias=False)	# x*W (no bias) #added by zhengquan[?,8] , 写的是inputs，但是他的实际含义是输出的维度。
    ...
    def load_data(self, train_dfn="/home3/data/zhengquan/from_huangbo/lr_feature_select/new_data/train_ins_continues.txtlineNUM_videoID",
    ...
    def load_test_data(self,test_dfn="/home3/data/zhengquan/from_huangbo/lr_feature_select/new_data/test_ins_continues.txtlineNUM_videoID"):
    ...
    def train(self, n_epoch=1000, snapshot_step=10, batch_size=None):
        ...
        print ("Input data shape = %s; output data shape=%s, batch_size=%s" % (str(self.X_dict['wide_X'].shape), 
                                                                               ...
                                                                               batch_size))
        print ("Eval data shape = %s; output data shape=%s, validation_batch_size=%s" % (str(self.evalX_dict['wide_X'].shape),
...
def CommandLine(args=None):
    ...
    if FLAGS.do_test:
        ...
        fpr, tpr, thresholds = metrics.roc_curve(y_true=gtruth, y_score=y_pred_prob,
                                                 pos_label=1)  # 因为我发现居然有2，train中3个，test中两个
        auc1 = metrics.auc(fpr, tpr)
        print("eval_auc = ", auc1)
        auc2 = sklearn.metrics.roc_auc_score(gtruth, list(y_pred_prob))
        print("eval_auc = ", auc2)
