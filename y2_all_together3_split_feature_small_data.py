...
class WideAndDeep(object):
    ...
    def train(self,train_filename="",eval_filename=""):
            ...
            for idx, current_batch_data in enumerate(batch_data):
                ...
                if train_steps % self.eval_freq == 0:
                    ...
                    if eval_auc > history_auc or (eval_auc == history_auc and eval_acc > history_acc) :
                        self.save_model(save_dir="/home2/data/zhengquan/WAD_filterEmbedding_1024_512_256_small_data/",prefix="auc=%.3f"%(eval_auc))
        ...
        print("epoch = %d, train_steps=%d, auc=%.3f, acc=%.3f" % (epoch, train_steps, eval_auc, eval_acc))
        if eval_auc > history_auc or (eval_auc == history_auc and eval_acc > history_acc):
            self.save_model(save_dir="/home2/data/zhengquan/small_data_batch_size_200_lr_08_bert/",
...
if __name__ == "__main__":
    ...
    A = WideAndDeep(batch_size=20,eval_freq=5000,feature_num=100,tag2value=tag2value,tag2valueOneline=tag2valueOneline,custom_tags = custom_tags,
                    train_epoch_num=1,
