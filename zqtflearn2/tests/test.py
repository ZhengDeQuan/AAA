'''
    This file contains test cases for zqtflearn
'''
...
class TestActivations(unittest.TestCase):
    '''
        This class contains test cases for the functions in zqtflearn/activations.py
    '''
    ...
    def test_leaky_relu(self):
        f = lambda x: zqtflearn.leaky_relu(x, alpha=0.2)
        x = tf.placeholder(tf.float32, shape=())
    ...
    def test_apply_activation(self):
        lrelu_02 = lambda x: zqtflearn.leaky_relu(x, alpha=0.2)
        x = tf.constant(-0.25, tf.float32)

        with tf.Session() as sess:
            # Case 1: 'linear'
            self.assertEqual(
                sess.run(zqtflearn.activation(x, 'linear')),
                -0.25)

            # Case 2: 'relu'
            self.assertEqual(
                sess.run(zqtflearn.activation(x, 'relu')),
                0)

            # Case 3: 'leaky_relu'
            self.assertAlmostEqual(
                sess.run(zqtflearn.activation(x, 'leaky_relu')),
                -0.025, places=TestActivations.PLACES)

            # Case 4: 'tanh'
            self.assertAlmostEqual(
                sess.run(zqtflearn.activation(x, 'tanh')),
                -0.2449, places=TestActivations.PLACES)

            # Case 5: lrelu_02 (callable)
            self.assertAlmostEqual(
                sess.run(zqtflearn.activation(x, lrelu_02)),
