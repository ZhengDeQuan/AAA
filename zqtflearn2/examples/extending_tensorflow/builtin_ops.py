...
with tf.Graph().as_default():
    ...
    def dnn(x):
        ...
        x = zqtflearn.prelu(tf.add(tf.matmul(x, W1), b1))
        zqtflearn.summaries.monitor_activation(x) # Monitor activation
        x = zqtflearn.prelu(tf.add(tf.matmul(x, W2), b2))
        zqtflearn.summaries.monitor_activation(x) # Monitor activation
    ...
    loss = zqtflearn.categorical_crossentropy(net, Y)

    # Using metric ops from TFLearn to compute accuracy
    acc = zqtflearn.metrics.accuracy_op(net, Y)

    # Using TFLearn SGD Optimizer class
    optimizer = zqtflearn.SGD(learning_rate=0.1, lr_decay=0.96, decay_step=200)
    ...
    step = zqtflearn.variable("step", initializer='zeros', shape=[])
    ...
    trainop = zqtflearn.TrainOp(loss=loss, optimizer=optim_tensor,
    ...
    trainer = zqtflearn.Trainer(train_ops=trainop, tensorboard_verbose=0)
