...
class TFLearnSeq2Seq(object):
    ...
    def model(self, mode="train", num_layers=1, cell_size=32, cell_type="BasicLSTMCell", embedding_size=20, learning_rate=0.0001,
        ...
        network = zqtflearn.input_data(shape=[None, self.in_seq_len + self.out_seq_len], dtype=tf.int32, name="XY")
        ...
        model = zqtflearn.DNN(network, tensorboard_verbose=tensorboard_verbose, checkpoint_path=checkpoint_path)
        return model
...
"""
    ...
    parser.add_argument("-i", "--input-weights", type=str, help="zqtflearn file with network weights to load", default=None)
    parser.add_argument("-o", "--output-weights", type=str, help="new zqtflearn file where network weights are to be saved", default=None)
