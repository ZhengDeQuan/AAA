...
from zqtflearn.vendor.arg_scope import add_arg_scope as contrib_add_arg_scope
...
def variable(name, shape=None, dtype=tf.float32, initializer=None,
    ...
    Arguments:
        ...
        initializer: `str` or `Tensor`. The variable initialization. (See
            zqtflearn.initializations for references).
        regularizer: `str` or `Tensor`. The variable regularizer. (See
            zqtflearn.losses for references).
        ...
        restore: `bool`. Restore or not this variable when loading a
            pre-trained model (Only compatible with zqtflearn pre-built
            training functions).
