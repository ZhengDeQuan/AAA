...
这种做法是不合理的。因为从2发展到了3.
需要多加一列.
因为增加了bert的embedding之后感觉效果并没有很好。
我怀疑是embedding的维度768比较大造成的。
我想对输入到deep侧的bert embedding进行预处理，将768维度的数据进行totate（扭曲）
用两层全连接，768 -- > 300 --> 100 然后再把100维的向量送入到 deep侧。
'''
...
class WideAndDeep(object):
    def __init__(self,
        ...
        self.sibling_side_nodes = sibling_side_nodes
        ...
        for tag in tags:
            if tag.wide_or_deep_side == "wide":
                ...
                if tag.sibling is not None and tag.sibling.wide_or_deep_side == "wide":
                    sibling_dimension_size += tag.sibling.embedding_size
            else:
    ...
    def _realize_mappings(self):
        with tf.device('/cpu:0'), tf.variable_scope('word_embedding'):
            ...
            for key in sibling_mappings:
                mappings[key] = sibling_mappings[key]

            tensors = wide_tensors + deep_tensors + sibling_tensors
            ...
            wide_inputs, deep_inputs, sibling_inputs = tf.split(wide_and_deep_embedding_res, [self.wide_side_dimension_size, self.deep_side_dimension_size, self.sibling_dimension_size],1)
            ...
            self.sibling_inputs = tf.reshape(sibling_inputs,[self.batch_size, self.sibling_dimension_size])
            print("wide_inputs.shape = ", self.wide_inputs.shape)
            print("deep_inputs.shape = ", self.deep_inputs.shape)
            print("sibling_input.shape = ",self.sibling_inputs.shape)
            '''
            wide_inputs.shape =  (10, 389)
            deep_inputs.shape =  (10, 2038 - 768)
            sibling_input.shape = (10, 768)
            '''
            # import pdb
            # pdb.set_trace()
    ...
    def _build_graph(self):
        ...
        #sibling part
        for j in range(len(self.sibling_side_nodes)):
            self.sibling_inputs = tf.contrib.layers.fully_connected(self.sibling_inputs,self.sibling_side_nodes[j],activation_fn=tf.nn.relu)
            self.sibling_inputs = tf.layers.dropout(
                inputs = self.sibling_inputs,
                rate=0.5,
                name="sibling_dropout_%d"%j
            )


        w_a_d = tf.concat([self.wide_inputs, self.deep_inputs,self.sibling_inputs], axis=1, name="concat")
        # deep part
        ...
        for epoch in range(self.train_epoch_num):
            ...
            for idx, current_batch_data_indices in enumerate(batch_data_indices):
                ...
                if train_steps % self.eval_freq == 0:
                    ...
                    print(time.strftime("%Y-%m-%d %H:%M:%S ", time.localtime()),"epoch = %d, train_steps=%d, auc=%.3f, acc=%.3f" % (epoch, train_steps, eval_auc, eval_acc))
                    if eval_auc > history_auc or (eval_auc == history_auc and eval_acc > history_acc):
...
if __name__ == "__main__":
    ...
    A = WideAndDeep(batch_size=200,eval_freq=1500,tag2value=tag2value,tag2valueOneline=tag2valueOneline,custom_tags = custom_tags,
                    train_epoch_num=1,
