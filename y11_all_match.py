...
class WideAndDeep(object):
    ...
    def _build_loss(self):
        ...
        sim_loss_user_video = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
            labels=self._Y,
        ...
        self.total_loss += self.sim_loss_a * sim_loss_user \
                           + self.sim_loss_b * sim_loss_video \
                           + self.sim_loss_c * sim_loss_context \
                           + self.sim_loss_ab * sim_loss_user_video
    ...
    def load_data(self,filename = "/home2/data/ttd/zhengquan_test.processed.csv.pkl",train=False):
    ...
    def train(self,train_filename="",eval_filename=""):
        if train_filename:
            self.X_data, self.Y_data = self.load_data(train_filename,train=True)
        else:
            self.X_data, self.Y_data = self.load_data(self.train_filename,train=True)
        # Test_Example_Num = 20
