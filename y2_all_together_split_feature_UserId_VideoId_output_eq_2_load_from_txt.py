...
class WideAndDeep(object):
    ...
    def _setup_mappings(self):
        ...
        for key in tag2value:
            print("in setup mapping key = ",key)
            tag = Tag(
    ...
    def _realize_mappings(self):
        ...
        for one_feature, tag in zip(features, self.tags):
            print("in realize mapping tag.name = ",tag.tag_name)
            if tag.wide_or_deep_side != "wide":
                continue
    ...
    def load_data_from_txt(self,filename):
        X_data = []
        Y_data = []
        ...
        import pdb
        pdb.set_trace()
        X_data = np.array(X_data,dtype=str)
    ...
    def train(self,train_filename="",eval_filename=""):
        if train_filename:
            self.X_data, self.Y_data = self.load_data_from_txt(train_filename)
        else:
            self.X_data, self.Y_data = self.load_data_from_txt(self.train_filename)
        # Test_Example_Num = 20
        ...
        if eval_filename:
            self.eval_X_data, self.eval_Y_data = self.load_data_from_txt(eval_filename)
        else:
            self.eval_X_data, self.eval_Y_data = self.load_data_from_txt(self.eval_filename)
        train_steps = 0
        history_acc = 0
        history_auc = 0
        start_t = time.time()
    ...
    def evaluate(self,filename=""):
        if self.eval_X_data is None:
            if filename:
                self.eval_X_data , self.eval_Y_data = self.load_data_from_txt(filename)
            else:
                self.eval_X_data , self.eval_Y_data = self.load_data_from_txt(self.eval_filename)
...
if __name__ == "__main__":
    ...
    # print("tag2value.keys()")
    # print(tag2value.keys())
    # exit(78)
    tag2valueOneline = json.load(open('original/tag2valueOneline.json', "r", encoding="utf-8"))
    A = WideAndDeep(batch_size=50,eval_freq=100,tag2value=tag2value,tag2valueOneline=tag2valueOneline,custom_tags = [],
                    train_epoch_num=2,
    ...
    A.train(train_filename="/home2/data/ttd/eval_ins_add.processed",eval_filename="/home2/data/ttd/eval_ins_add.processed")
