...
def conv_2d(incoming, nb_filter, filter_size, strides=1, padding='same',
    ...
    Arguments:
        ...
        activation: `str` (name) or `function` (returning a `Tensor`) or None.
            Activation applied to this layer (see zqtflearn.activations).
            Default: 'linear'.
        bias: `bool`. If True, a bias is used.
        weights_init: `str` (name) or `Tensor`. Weights initialization.
            (see zqtflearn.initializations) Default: 'truncated_normal'.
        bias_init: `str` (name) or `Tensor`. Bias initialization.
            (see zqtflearn.initializations) Default: 'zeros'.
        regularizer: `str` (name) or `Tensor`. Add a regularizer to this
            layer weights (see zqtflearn.regularizers). Default: None.
...
def residual_block(incoming, nb_blocks, out_channels, downsample=False,
    ...
    Arguments:
        ...
        bias: `bool`. If True, a bias is used.
        weights_init: `str` (name) or `Tensor`. Weights initialization.
            (see zqtflearn.initializations) Default: 'uniform_scaling'.
    ...
    with tf.variable_scope(scope, default_name=name, values=[incoming],
        ...
        for i in range(nb_blocks):
            ...
            if batch_norm:
                resnet = zqtflearn.batch_normalization(resnet)
            resnet = zqtflearn.activation(resnet, activation)
            ...
            if downsample_strides > 1:
                identity = zqtflearn.avg_pool_2d(identity, downsample_strides,
                                                 downsample_strides)
...
def resnext_block(incoming, nb_blocks, out_channels, cardinality,
    ...
    with tf.variable_scope(scope, default_name=name, values=[incoming],
        ...
        for i in range(nb_blocks):
            ...
            resnext = zqtflearn.activation(resnext, activation)
...
def densenet_block(incoming, nb_layers, growth, bottleneck=True,
    ...
    with tf.variable_scope(scope, default_name=name, values=[incoming],
        ...
        for i in range(nb_layers):
            ...
            if bottleneck:
                if batch_norm:
                    densenet = zqtflearn.batch_normalization(densenet)
                densenet = zqtflearn.activation(densenet, activation)
        ...
        densenet = conv_2d(densenet, nb_filter=growth,
                           ...
                           restore=restore)
        if dropout:
            densenet = zqtflearn.dropout(densenet, keep_prob=dropout_keep_prob)

        # Downsampling
        if downsample:
            densenet = zqtflearn.avg_pool_2d(densenet, kernel_size=2,
