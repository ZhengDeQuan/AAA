...
class ZQDNN(object):
    ...
    def __init__(self, network, clip_gradients=5.0, tensorboard_verbose=0,
        ...
        self.net = network
        print("custom train op = ",train_op)
        self.train_ops = tf.get_collection(tf.GraphKeys.TRAIN_OPS) if train_op is None else train_op
        print("in ZQDNN")
        print("tf.GraphKeys.TRAIN_OPS = ",tf.GraphKeys.TRAIN_OPS)
        print("train_ops = ",self.train_ops)
        exit(89)
