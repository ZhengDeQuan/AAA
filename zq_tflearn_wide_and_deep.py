...
class TFLearnWideAndDeep(object):
    ...
    def build_model(self, learning_rate=[0.001, 0.01]):
        ...
        n_categories = len(self.categorical_columns)
        ...
        deep_inputs = zqtflearn.input_data(shape=input_shape, name="deep_X")
        ...
        if 'deep' in self.model_type:
            deep_network = self.deep_model(wide_inputs, n_cc) #这里面是wide inputs,在这个函数内部wide_inputs,会和deep_model制造的输入相合并。
        ...
        if 'deep' in self.model_type:
            ...
            if category_map is None:
                ...
                for cc in self.categorical_columns:
                    ...
                    # print(td[cc])
                    # print("zhengquan"*88)
                    # print(td[cc].unique()) #['State-gov' 'Self-emp-not-inc' 'Private' 'Federal-gov' 'Local-gov' '?','Self-emp-inc' 'Without-pay' 'Never-worked']
                    # print("zhengquan"*88)
                    # print(sorted(td[cc].unique()))#['?', 'Federal-gov', 'Local-gov', 'Never-worked', 'Private', 'Self-emp-inc', 'Self-emp-not-inc', 'State-gov', 'Without-pay']
                    #
                    cc_values = sorted(td[cc].unique())
                    cc_max = 1+len(cc_values)
                    cc_map = dict(zip(cc_values, range(1, cc_max)))	# start from 1 to avoid 0:0 mapping (save 0 for missing)万一将来有一个item在这个cotegory上 没有的值该怎么办
                    if self.verbose:
                        print ("  category %s max=%s,  map=%s" % (cc, cc_max, cc_map))
                    category_map[cc] = cc_map
                
            td = td.replace(category_map) #df.replace(to_replace, value) 前面是需要替换的值，后面是替换后的值，进行上述操作之后，其实原DataFrame是并没有改变的。改变的只是一个复制品。如果需要改变原数据，需要添加常用参数 inplace=True，用字典形式替换多个值，{'C':0.999, 'F':0.777}，字典里面的键‘C'和'F'就是原值，字典里的值0.999和0.777就是我们想要的新值。
