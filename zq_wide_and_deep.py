...
import sys
import argparse
# from zqtflearn2 import zqtflearn
# import zqtflearn as tflearn
import zqtflearn as tflearn
...
# 数据格式，每行109列，第一列是label，之后的108列是特征值。先用wide模型跑
...
class TFLearnWideAndDeep(object):
    ...
    def build_model(self, learning_rate=[0.001, 0.01]):
        ...
        deep_inputs = tflearn.input_data(shape=[None, 1], name="deep_X")
        ...
        self.model = tflearn.ZQDNN(network,
                                 tensorboard_verbose=self.tensorboard_verbose,
                                 max_checkpoints=400,
                                 checkpoint_path="%s/%s.tfl" % (self.checkpoints_dir, self.name),
                                 tensorboard_dir="bert_tflearn_logs/",
                                 train_op = "I will learn to use it"
                                 )
        # tensorboard_dir="/tmp/tflearn_logs/" zqtflearn.DNN 我把他改为当前目录下的了，这样也比较好规范
        if 'deep' in self.model_type:
            embeddingWeights = tflearn.get_layer_variables_by_name('deep_video_ids_embed')[0]
    ...
    def deep_model(self, wide_inputs, deep_inpus, n_inputs, n_nodes=[100, 50], use_dropout=False):
        '''
        ...
        net = tflearn.layers.embedding_ops.embedding(deep_inpus, len(self.vocab), self.embedding.shape[1],
                                                     trainable=False
                                                     , name="deep_video_ids_embed")
        network = tf.concat([wide_inputs, tf.squeeze(net, squeeze_dims=[1], name="video_ids_squeeze")], axis=1,
        ...
        network = tflearn.fully_connected(network, 1, activation="linear", name="deep_fc_output", bias=False)  # [?,1]
        network = tf.reshape(network, [-1,
                                       1])  # so that accuracy is binary_accuracy added by zhengquan ,不reshape不也是[?,1]的吗?可能如果最后的输出维度是1的话，结果是[?]的尺寸
        if self.verbose:
    ...
    # def load_data(self,
    #               train_dfn="train_ins_continues.txtlineNUM_videoID",
    #               eval_dfn="eval_ins_continues.txtlineNUM_videoID"):
        ...
        self.train_data = self.train_data.sample(frac=1).reset_index(
            drop=True)  # shuffle [以100%de比例随机选择原来的数据，drop=True自动新建一列记录原来的index]
        ...
        self.train_data = self.train_data[
            self.train_data[self.label_column].isin([0, 1])]  # 防止label列，即第0列，有除了0，1之外的值出现。
        self.eval_data = self.eval_data[self.eval_data[self.label_column].isin([0, 1])]

        if "deep" in self.model_type:
            col_num = self.train_data.shape[1] - 1
            self.train_data[col_num] = (
                self.train_data[col_num].apply(
                    lambda x: self.vocab[x] if x in self.vocab else self.vocab["[UNK]"])).astype(int)
            self.eval_data[col_num] = (
                self.eval_data[col_num].apply(
                    lambda x: self.vocab[x] if x in self.vocab else self.vocab["[UNK]"])).astype(int)

    def load_test_data(self,
                       test_dfn="/home3/data/zhengquan/from_huangbo/lr_feature_select/new_data/test_ins_continues.txtlineNUM_videoID"):
    # def load_test_data(self,
    #                    test_dfn="test_ins_continues.txtlineNUM_videoID"):
...
def CommandLine(args=None):
    ...
    try:
        ...
        flags.DEFINE_string("checkpoints_dir", "C_shuf", "name of directory where checkpoints should be saved")
        flags.DEFINE_integer("n_epoch", 200, "Number of training epoch steps")
        flags.DEFINE_integer("batch_size", 5, "Number of items in one batch")
    ...
    if FLAGS.do_train:
        twad.train(n_epoch=FLAGS.n_epoch, snapshot_step=FLAGS.snapshot_step, batch_size=FLAGS.batch_size)
    ...
    print("eval_auc = ", auc3) #eval_auc =  0.534072249589491 bert向量加持的随机初始化的向量的结果（小数据集）。 eval_auc =  0.47865353037766833 wide模型的结果
                               #val_auc =  0.5233990147783252 （小数据集，bert，train了2epoch）
